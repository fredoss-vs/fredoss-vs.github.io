...
module.exports = NodeHelper.create({
    ...
    formatResults (ret, payload) {
        ...
        if (!this.instanceArticles[instanceId]) { this.instanceArticles[instanceId] = [] }
        const results = []
        ...
        if (results.length > 0) { this.instanceArticles[instanceId] = this.instanceArticles[instanceId].concat(results) }
        if (payload.debug) { Log.log("sending articles: ", JSON.stringify(this.instanceArticles[instanceId])) }
        this.sendSocketNotification("NEWSAPI_UPDATE", {instanceId: instanceId, articles: this.instanceArticles[instanceId]})
    },
